...
Program ImperativoClase4_ejercicio1;
...
{ Retornar la cantidad de códigos que existen en el árbol que son menores que un valor que se recibe como parámetro }
  ...
  begin
    if (a = nil) 
    then CantidadDeCodigosMenores:=0
    else begin
		if(a^.dato.codigo < cod)then CantidadDeCodigosMenores := 1 + CantidadDeCodigosMenores(a^.HI,cod) + CantidadDeCodigosMenores(a^.HD,cod)
		else CantidadDeCodigosMenores := CantidadDeCodigosMenores(a^.HI,cod) + CantidadDeCodigosMenores(a^.HD,cod);
    end;
  end;
...
comprendidos entre los dos valores recibidos (sin incluir). }
  ...
  begin
    if (a = nil) 
    then ObtenerMontoTotalEntreDosCodigos:=0
    else begin
		if(a^.dato.codigo > codigo1)and(a^.dato.codigo < codigo2)then ObtenerMontoTotalEntreDosCodigos := a^.dato.montoTotal + ObtenerMontoTotalEntreDosCodigos(a^.HI,codigo1,codigo2) + ObtenerMontoTotalEntreDosCodigos(a^.HD,codigo1,codigo2)
		else ObtenerMontoTotalEntreDosCodigos := ObtenerMontoTotalEntreDosCodigos(a^.HI,codigo1,codigo2) + ObtenerMontoTotalEntreDosCodigos(a^.HD,codigo1,codigo2);
    end;
  end;
...
begin
  ...
  then writeln ('No hay codigos entre ', codigo1, ' y ', codigo2)
         ...
         writeln;
         write('El monto total entre el codigo ', codigo1, ' y el codigo : ', codigo2); 
         writeln(' es: ', montoTotal :2:0);		//Devuelve valor redondeado , ajustarlo opcionalmente.
         writeln;
       end;
  writeln;
  writeln ('-----------------------------------------------');
