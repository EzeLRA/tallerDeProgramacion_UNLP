...
function buscarNombre(a: arbol; nom:String):boolean; //modificar
{Busca el nombre}
begin
	if (a = nil)then buscarNombre:= false
  else begin
		buscarNombre:= (a^.dato.nombre = nom)or(buscarNombre(a^.HI,nom))or(buscarNombre(a^.HD,nom));
  end;
end;

procedure InformarExistenciaNombreSocio(a:arbol; nom:String);
begin
	writeln;
	write('Resultado: ');
	writeln(buscarNombre(a,nom));
	writeln;
end;
...
function edadesTotal(a:arbol):integer;
begin
	if (a = nil) 
    then edadesTotal:= 0
    else begin        
		edadesTotal:= a^.dato.edad + edadesTotal(a^.HI) + edadesTotal(a^.HD);
    end;
end; 
...
begin
	...
	writeln('Promedio de Edad: ', total/cant);
end;
